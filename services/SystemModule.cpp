...
, m_versionChannel(versionChannel) {
    HAL_GPIO_ReadPin(fixtureID1_GPIO_Port, fixtureID1_Pin) == GPIO_PIN_RESET ? SET_BIT(m_fixtureId, System::IdBit1)
                                                                             : CLEAR_BIT(m_fixtureId, System::IdBit1);
    HAL_GPIO_ReadPin(fixtureID2_GPIO_Port, fixtureID2_Pin) == GPIO_PIN_RESET ? SET_BIT(m_fixtureId, System::IdBit2)
                                                                             : CLEAR_BIT(m_fixtureId, System::IdBit2);
    HAL_GPIO_ReadPin(fixtureID3_GPIO_Port, fixtureID3_Pin) == GPIO_PIN_RESET ? SET_BIT(m_fixtureId, System::IdBit3)
...
void SystemModule::Run() {
    ...
    m_status = (HAL_GPIO_ReadPin(COVER_IN_GPIO_Port, COVER_IN_Pin)) ? m_status | 0x8000 : m_status & 0x7FFF;
    ...
    if (umo->IsUniverseReady(m_universeId)) {
        ...
        umo->SetChannels(m_universeId, m_statusStartChannel, {(uint8_t)(m_status >> 8), (uint8_t)(m_status & 0x00FF)});

        // Update serial number in universe. 102
        umo->SetChannels(m_universeId, m_snStartChannel, {(uint8_t)(m_sn >> 8), (uint8_t)(m_sn & 0x00FF)});
...
void SystemModule::SetStatus(System::SystemStatus status) {
    ...
    // If status is OK:
        ...
        CLEAR_BIT(
          m_status,
          System::SystemStatus::HalError | System::SystemStatus::DriverError | System::SystemStatus::InterfaceError
            | System::SystemStatus::ServiceError | System::SystemStatus::ProcessError | System::SystemStatus::PostError
