 ...
 * --------------------------------------------------------------- */
SpiModule::SpiModule(SPI_HandleTypeDef* handle, const std::string& label) : m_label(label), m_handle(handle) {
...
CEP_SPI::Status SpiModule::Transmit(const std::vector<uint8_t> pkt) { return Transmit(pkt.data(), pkt.size()); }
...
CEP_SPI::Status SpiModule::Transmit(const uint8_t* data, size_t len) {
    ...
    if (HAL_SPI_Transmit(m_handle, const_cast<uint8_t*>(data), (uint16_t)len, SpiModule::TIMEOUT) != HAL_OK) {
        m_status |= (CEP_SPI::Status)m_handle->ErrorCode;
        ErrorHandler();
        return m_status;
    }
...
CEP_SPI::Status SpiModule::Transaction(const uint8_t* txData, size_t txLen, uint8_t* rxData, size_t rxLen) {
    ...
    if (
      HAL_SPI_TransmitReceive(m_handle, const_cast<uint8_t*>(txData), rxData, (uint16_t)txLen, SpiModule::TIMEOUT)
      != HAL_OK) {
        m_status |= (CEP_SPI::Status)m_handle->ErrorCode;
        ErrorHandler();
        return m_status;
...
CEP_SPI::Status SpiModule::Transaction(const std::vector<uint8_t>& txData, std::vector<uint8_t>& rxData) {
