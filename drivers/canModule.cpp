...
CanModule::CanModule(CAN_HandleTypeDef* handle, std::string label) : m_handle(handle), m_label(std::move(label)) {
    CEP_ASSERT(handle != nullptr, "CAN Handle is NULL!");
    m_framesReceived.reserve(8);
    m_callbacks = std::map<CEP_CAN::Irq, std::function<void()>>({{CEP_CAN::Irq::TxMailboxEmpty, {}},
