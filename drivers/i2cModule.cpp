...
I2cModule::I2cModule(I2C_HandleTypeDef* handle, std::string label) : m_handle(handle), m_label(std::move(label)) {
...
void I2cModule::TransmitFrame(uint8_t addr, const uint8_t* data, size_t len) {
    if (
      HAL_I2C_Master_Transmit(m_handle, addr, const_cast<uint8_t*>(data), (uint16_t)len, I2cModule::TIMEOUT)
      != HAL_OK) {
        LOGTE(m_label.c_str(), "In TransmitFrame, unable to transmit frame");
    }
}

void I2cModule::TransmitFrameToRegister(uint8_t addr, uint8_t regAddr, const uint8_t* data, size_t len) {
    if (
      HAL_I2C_Mem_Write(
        m_handle,
        addr,
...
CEP_I2C::Frame I2cModule::ReceiveFrame(uint8_t addr, size_t len) {
    ...
    if (
      HAL_I2C_Master_Receive(m_handle, addr, frame.data.data(), (uint16_t)frame.data.size(), I2cModule::TIMEOUT)
      != HAL_OK) {
