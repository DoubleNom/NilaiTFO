...
namespace CEP_RTC {
Time::Time(const RTC_TimeTypeDef& time) : hours(time.Hours), minutes(time.Minutes), seconds(time.Seconds) {
...
RtcModule::RtcModule(RTC_HandleTypeDef* handle, const std::string& label) : m_label(label), m_handle(handle) {
...
CEP_RTC::Timestamp RtcModule::GetTimestamp() { return CEP_RTC::Timestamp(GetDate(), GetTime()); }

size_t RtcModule::GetEpoch() { return GetEpoch(GetDate(), GetTime()); }
